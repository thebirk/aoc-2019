...
part1 :: proc(input: string) {
    ...
    parse_entry :: proc(input: string) -> (string, string, string) {
        ...
        planet := start[:len(start)-len(input)];
        ...
        orbiter := start[:len(start)-len(input)];

        if input != "" && input[0] == '\n' do input = input[1:];

        return input, planet, orbiter;
    }

    parse_input :: proc(input: string) -> ^Node {
        input := input;
        ...
        for input != "" {
            planet, orbiter: string;
            input, planet, orbiter = parse_entry(input);

            child: ^Node;
            if n, ok := names[orbiter]; ok {
                child = n;
            } else {
                child = make_node(orbiter);
                names[orbiter] = child;
            }

            if n, ok := names[planet]; ok {
                child.parent = n;
                append(&n.children, child);
            } else {
                parent := make_node(planet);
                child.parent = parent;
...
part2 :: proc(input: string) {
    ...
    parse_input :: proc(input: string) -> (^Node, ^Node) {
        input := input;
